...
var db = module.parent.parent.require('./database'),
	...
	url = require.main.require('url'),

	Auth = {};

Auth.mapExternalId = function(req, res, cb) {
	var qs = url.parse(req.url, true).query;
  db.getObjectField(qs.app + 'Id:uid', req.params.uid, function(err, uid) {
    if (err) return cb(err);
    else return cb(null, uid);
  });
};
